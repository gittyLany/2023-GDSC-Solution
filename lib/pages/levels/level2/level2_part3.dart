...
import 'package:turtle/pages/levels/level2/level2_part4.dart';
...
class Level2Part3Screen extends StatefulWidget {
  /// Discussion from plastic bag mini game to scene change
  const Level2Part3Screen({super.key});

  @override
  State<Level2Part3Screen> createState() => _Level2Part3State();
}

class _Level2Part3State extends State<Level2Part3Screen> {
  int _textIndex = Level2Dialog.minigame1BreakIndex;
  bool _dialogShow = true;

  @override
  Widget build(BuildContext context) {
    // Cancel state
    if (_textIndex == Level2Dialog.sceneChangeIndex && _dialogShow) {
      setState(() {
        _dialogShow = false;
      });
    }
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) => const TurtleLoadingScreen(
                    nextScreen: Level2Part4Screen())));
      });
    }

    if (_dialogShow) {
      if (_textIndex == Level2Dialog.imageIndex) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void _showDialogImage(BuildContext context) {
    MyDialog dialog = Level2Dialog.dialogs[_textIndex];
